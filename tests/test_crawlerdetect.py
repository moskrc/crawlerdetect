...
class CrawlerDetectTests(CrawlerDetectTestCase):
    ...
    def test_is_crawler(self):
        res = self.cd.isCrawler('Mozilla/5.0 (iPhone; CPU iPhone OS 7_1 like Mac OS X) AppleWebKit (KHTML, like Gecko) Mobile (compatible; Yahoo Ad monitoring; https://help.yahoo.com/kb/yahoo-ad-monitoring-SLN24857.html)')
        self.assertTrue(res)

    def test_user_agents_are_bots(self):
        with open(os.path.join(os.path.dirname(__file__), 'fixtures/crawlers.txt'), 'r') as f:
            for line in f:
                test = self.cd.isCrawler(line)
                self.assertTrue(test, line)

    def test_user_agents_are_devices(self):
        with open(os.path.join(os.path.dirname(__file__), 'fixtures/devices.txt'), 'r') as f:
            for line in f:
                test = self.cd.isCrawler(line)
                self.assertFalse(test, line)

    def test_it_returns_correct_matched_bot_name(self):
        self.cd.isCrawler('Mozilla/5.0 (iPhone; CPU iPhone OS 7_1 like Mac OS X) AppleWebKit (KHTML, like Gecko) Mobile (compatible; Yahoo Ad monitoring; https://help.yahoo.com/kb/yahoo-ad-monitoring-SLN24857.html)')
